class Day4
	...
	def problems_solution(input)
		...
		boards.each_with_index do |board, board_index|
			size = board.size
			winner_lines_winning_indices = Array.new
		...
		first_winner_data = winners.min_by { |board_idx, winner_idx| winner_idx }
		last_winner_data = winners.max_by { |board_idx, winner_idx| winner_idx }		

		puts "problem 1 answer: #{get_solution_for_winner_data(first_winner_data, boards, winning_numbers)}" # correct answer 39902
		puts "problem 1 answer: #{get_solution_for_winner_data(last_winner_data, boards, winning_numbers)}" # correct answer 26936
	end	
